...
export const EARS_PagarCuota = async ({id_cuota, valor}) => {
    ...
    const [movi] = await coneccion.query(`insert into movimientos (fecha, valor, tipo, estado) values (now(), ?, 'ingreso', 'activo')`, [valor]);
    const id_movimiento = movi.insertId;
    await coneccion.query(`insert into pagos(cuota, valor, fecha, movimiento) values (?,?, now(),?)`, [id_cuota, valor, id_movimiento])
    const nuevo_total = total_pagado + valor;
